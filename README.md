...
Обязательными для запуска `install.sh` с последующим стартом docker compose считаются только:
...
- после заполнения обязательных токенов тот же `install.sh` запускается повторно и поднимает уже собранный стек без лишней пересборки
...
9. если обязательные токены уже заполнены:
   - при первом запуске без готовых образов выполнить `docker compose up -d --build`
   - при повторном запуске с уже собранными образами выполнить быстрый старт `docker compose up -d`
...
4. если образы уже существуют, выполнить быстрый старт `docker compose up -d`
5. если образов ещё нет, выполнить первый запуск с `docker compose up -d --build`
...
### Быстрый повторный запуск без пересборки

Если код и Dockerfile не менялись, для следующего старта достаточно:
...
Если менялись исходники, зависимости или Dockerfile, пересобирайте стек вручную:
...
Если код не менялся и нужен только быстрый повторный старт уже собранного стека:
...
./install.sh  # быстрый повторный старт, если образы уже собраны
docker compose up -d --build  # вручную, если менялись код, зависимости или Dockerfile
